...
impl CPU {
    ...
    fn php(&mut self) {
        self.stack_push(self.status.to_u8());
    }
...
mod test {
    ...
    #[test]
    fn test_status_get_bits() {
        let mut status = Status::default();
        status.zero_flag = true;

        assert_eq!(status.to_u8(), 0b0000_0010);

        status.carry_flag = true;
        status.negative_flag = true;

        assert_eq!(status.to_u8(), 0b1000_0011);
    }
    ...
    fn test_php() {
        ...
        assert_eq!(cpu.stack_pop(), 0b0000_0001);
    }
}
